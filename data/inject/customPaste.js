async function performPasteByTyping() {
    ...
    try {
        ...
        if (!clipText && window.BussyAnandPassClipboard) {
            clipText = window.BussyAnandPassClipboard;
            clipboardSource = 'BussyAnandPassClipboard';
            console.log('[PasteByTyping] Using BussyAnandPassClipboard:', clipText.substring(0, 100));
        }
...
async function performDragDropPaste() {
    ...
    try {
        ...
        if (!clipText && window.BussyAnandPassClipboard) {
            clipText = window.BussyAnandPassClipboard;
            clipboardSource = 'BussyAnandPassClipboard';
            console.log('[DragDropPaste] Using BussyAnandPassClipboard:', clipText.substring(0, 100));
        }
    ...
    if (ctrlKey && !event.shiftKey && !event.altKey && (event.key === 'V' || event.key === 'v')) {
        ...
        if (activeElement && (activeElement.isContentEditable || activeElement.tagName === 'INPUT' || activeElement.tagName === 'TEXTAREA')) {
            ...
            try {
                ...
                // If empty, fall back to BussyAnandPassClipboard
                if (!clipText && window.BussyAnandPassClipboard) {
                    clipText = window.BussyAnandPassClipboard;
                    console.log('[Paste] Using BussyAnandPassClipboard');
                }
