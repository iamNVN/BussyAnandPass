...
// Function to wait for page to load and question to appear
async function waitForQuestionToLoad(maxWaitTime = 10000) {
    const startTime = Date.now();
    console.log('[BussyAnandPass] Waiting for question to load...');

    while (Date.now() - startTime < maxWaitTime) {
        // Check if question data is present
        const questionElement = document.querySelector('div[aria-labelledby="question-data"]');
        if (questionElement && questionElement.innerText && questionElement.innerText.trim() !== '' && !questionElement.innerText.includes('Loading')) {
            console.log('[BussyAnandPass] Question loaded successfully');
            return true;
        }
    ...
    console.warn('[BussyAnandPass] Timeout waiting for question to load');
    return false;
}

// Function to convert image URL to Base64 object with timeout
// Function to convert img element to Base64 using Canvas API
async function convertImageToBase64(imgElement) {
    console.log(`[BussyAnandPass] Converting image from element:`, imgElement.src);
    try {
        ...
        console.log(`[BussyAnandPass] Successfully converted image (${canvas.width}x${canvas.height})`);
        ...
        console.error('[BussyAnandPass] Error converting image:', error);
        return null;
    }
}
...
async function extractQuestionCodeAndOptions() {
...
async function handleQuestionExtraction() {
    ...
    if (!loaded) {
        console.error('[BussyAnandPass] Question failed to load');
        return;
    }

    const { question, code, options } = await extractQuestionCodeAndOptions();
...
async function extractCodingQuestion() {
    console.log('[BussyAnandPass] extractCodingQuestion started');
    ...
    // Insert "Fetching..." text into the editor immediately for feedback
    window.dispatchEvent(new CustomEvent('BussyAnandPass_INSERT_CODE', {
        detail: { code: '// analyzing question and images...' }
    }));
    ...
    const extractWhitelistBlacklist = () => {
        const whitelist = [];
        const blacklist = [];

        // Find instruction cards
        const instructionCards = document.querySelectorAll('div[aria-labelledby="instruction-card"]');

        instructionCards.forEach(card => {
            const header = card.querySelector('div[aria-labelledby="instruction-header"]');
            if (!header) return;
            ...
            const valueCards = card.querySelectorAll('div[aria-labelledby="list-value-card"]');
            valueCards.forEach(v => values.push(v.innerText.trim()));

            if (headerText.includes('whitelist')) {
                whitelist.push(...values);
            } else if (headerText.includes('blacklist')) {
    ...
    const { whitelist, blacklist } = extractWhitelistBlacklist();
    console.log('[Constraints] Whitelist:', whitelist, 'Blacklist:', blacklist);
...
async function solveIamneoExamly() {
    chrome.storage.local.get(['funMode'], (data) => {
        if (data.funMode) {
            console.log('[Fun Mode] Alt+Shift+A triggered in content.js');
            // Send message to background to get next audio file
            chrome.runtime.sendMessage({ action: 'getFunModeAudio' }, (response) => {
                if (response && response.audioFile) {
                    setTimeout(() => {
                        const audioPath = chrome.runtime.getURL(response.audioFile);
    ...
    // improved detection: input-format, output-format, or presence of Ace editor related classes
    const codingQuestionElement = document.querySelector('div[aria-labelledby="input-format"]') ||
        document.querySelector('.ace_editor') ||
        document.querySelector('div[aria-labelledby="output-format"]');

    // Also check if text contains "Input Format" or "Output Format"
    const hasCodingText = document.body.innerText.includes("Input Format") || document.body.innerText.includes("Output Format");

    if (codingQuestionElement || hasCodingText) {
        console.log('Detected CODING question');
        await extractCodingQuestion();
    } else {
        console.log('Detected MCQ question');
        await handleQuestionExtraction();
    }
}
document.addEventListener('keydown', (event) => {
    ...
    if (modifierKey && event.shiftKey && event.code === 'KeyA') {
        // Play Fun Mode audio if enabled
...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
    ...
    if (message.action === 'solveIamneoExamly') {
        const solve = async () => { await solveIamneoExamly(); };
        solve();
    }
    if (message.action === 'solveMCQOnly') {
        handleQuestionExtraction();
    }
    if (message.action === 'updateEditorStatus') {
        // Update the editor with status message
        window.dispatchEvent(new CustomEvent('BussyAnandPass_INSERT_CODE', {
...
// Chat interface removed as per user request
// chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
        ...
        (async () => {
            try {
                ...
                if (request.isHackerRank) {
                    ...
                    // Match single letter (A-D) or digit
                    const optionMatch = request.response.match(/(?:options?\s*)?([A-D]|[1-4])\.?/i);
                    if (optionMatch) {
                        let optionNumber;
                        const match = optionMatch[1].toUpperCase();

                        // Map Letters to Indices
                        if (['A', 'B', 'C', 'D'].includes(match)) {
                            optionNumber = match.charCodeAt(0) - 65; // A=0, B=1, ...
                        } else {
                            optionNumber = parseInt(match) - 1; // 1=0, 2=1, ...
                        }
...
window.addEventListener('BussyAnandPass_REQUEST_CODE_TYPED', function (event) {
    const { programmingLanguage, question, inputFormat, outputFormat, testCases } = event.detail;

    console.log('[content.js] Received BussyAnandPass_REQUEST_CODE_TYPED event');
    ...
    chrome.runtime.sendMessage({
        ...
        if (response && response.success && response.response) {
            try {
                ...
                window.dispatchEvent(new CustomEvent('BussyAnandPass_INSERT_CODE_TYPED', {
                    detail: { code: cleanedResponse }
                }));

                console.log('[content.js] Dispatched BussyAnandPass_INSERT_CODE_TYPED event to exam.js');
            } catch (error) {
